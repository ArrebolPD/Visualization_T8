...
def import_csv(request):
    try:
        csv_file_path = 'data_ggsheet.csv'
        with open(csv_file_path, 'r', encoding='utf-8-sig') as file:
            reader = csv.DictReader(file)
            for row in reader:
                print(f"DEBUG: Thời gian tạo đơn = {row.get('Thời gian tạo đơn', 'NULL')}")
               
                order_time_str = row.get('Thời gian tạo đơn', '').strip()
                if not order_time_str:
                    return HttpResponse("Lỗi: Dữ liệu 'Thời gian tạo đơn' bị rỗng!", status=400)
               
                try:
                    order_time = datetime.strptime(order_time_str, '%Y-%m-%d %H:%M:%S')
                except ValueError as e:
                    return HttpResponse(f"Lỗi định dạng ngày: {order_time_str} ({str(e)})", status=400)
               
                name = row.get('Tên khách hàng', '').strip() or None
                customer, _ = Customer.objects.get_or_create(
                    customer_id=row['Mã khách hàng'],
                    defaults={'name': name, 'segment_code': row.get('Mã PKKH', '').strip()}
                )
               
                group, _ = ProductGroup.objects.get_or_create(
                    group_code=row['Mã nhóm hàng'],
                    defaults={'group_name': row.get('Tên nhóm hàng', '').strip()}
                )
               
                try:
                    unit_price = int(row.get('Đơn giá', '0').replace(',', ''))
                except ValueError:
                    unit_price = 0
                product, _ = Product.objects.get_or_create(
                    product_code=row['Mã mặt hàng'],
                    defaults={
                        'name': row.get('Tên mặt hàng', '').strip(),
                        'group': group,
